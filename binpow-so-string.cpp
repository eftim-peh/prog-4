...
public:
    ...
    pair<BigInt, BigInt> divmod(BigInt other) {
        if (other.value == "0") return {BigInt("Error"), BigInt("Error")};
        ...
        for (char c : value) {
            ...
            while (!(current.isLess(other))) {
                ...
                if (current.value == "0") currentStr = "";
                else currentStr = current.value;
                count++;
            }
            res += (char)(count + '0');
        }
        ...
        string q;
        if (first == string::npos) q = "0";
        else q = res.substr(first);
        
        string r;
        if (currentStr.empty()) r = "0";
        else r = currentStr;
